...
class Tools extends BaseTools
{
     ...
     * @var string
     */
    protected $urlPortal = 'http://www.cte.fazenda.gov.br/';
    ...
    protected $soapnamespaces = [
        ...
        'xmlns:cted' => "http://www.portalfiscal.inf.br/cte/wsdl/CTeDistribuicaoDFe"
    ];
    ...
    protected function sendRequest($request)
    {
        ...
        if (empty($this->oSoap)) {
            $this->oSoap = new SoapCurl($this->oCertificate);
        }
    ...
    /*
        Método de Distribuição de documentos eletronicos CT-e
    */

    public function DistFeCTe($ultNSU = '000000000000000'){

        $uf = $this->aConfig['siglaUF'];

        if (isset($this->cUFlist[$uf])){
            $codeUf = $this->cUFlist[$uf];
        } else {
            $msg = "O estado " . $uf . ' não existe na lista de estados';
        ...
        if ($cnpj == '')
            throw new Exception\RuntimeException('O CNPJ indicado esta vazio');

        $find = array($this->aConfig['tpAmb'],  $codeUf, $cnpj, $ultNSU);
        
        $replace = array('%{tpAmb}', '%{cUFAutor}','%{CNPJ}', '%{ultNSU}');

        $xml =  '<cted:cteDistDFeInteresse xmlns="http://www.portalfiscal.inf.br/cte/wsdl/CTeDistribuicaoDFe">'.
                '<cted:cteDadosMsg>' .
        ...
        $xml = str_replace($replace,$find, $xml);

        $this->method = "cteDistDFeInteresse";
        
        $this->action = "http://www.portalfiscal.inf.br/cte/wsdl/CTeDistribuicaoDFe/cteDistDFeInteresse";

        $this->uri = 'https://www1.cte.fazenda.gov.br/CTeDistribuicaoDFe/CTeDistribuicaoDFe.asmx';

        $this->urlService = 'https://www1.cte.fazenda.gov.br/CTeDistribuicaoDFe/CTeDistribuicaoDFe.asmx';

        $response = $this->sendRequest($xml);

        $response = substr($response, strpos($response, "<cteDistDFeInteresseResponse"));

        $response = substr($response, 0, strpos($response, "</cteDistDFeInteresseResponse>") + 30);
...
//        }
    ...
    public function zSefazEvento(
        ...
        // $signedMsg = $this->oCertificate->sign($cons, 'infEvento');
        $signedMsg = $cons;
        //limpa o xml
        $signedMsg = Strings::clearXmlString($signedMsg, true);
        //montagem dos dados da mensagem SOAP
        $body = "<cteDadosMsg xmlns=\"$this->urlNamespace\">$signedMsg</cteDadosMsg>";
    ...
    private function zTpEv($tpEvento = '')
    {
        ...
        switch ($tpEvento) {
            ...
            case '610110':
                ...
                $aliasEvento = 'EvPrestDesacordo';
                $descEvento = 'Servico em desacordo';
                break;
            default:
